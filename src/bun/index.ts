...
[APP_DATA_DIR, PYTHON_DIR, MODELS_DIR, VOICES_DIR, OUTPUT_DIR].forEach((dir) => {
  if (!existsSync(dir)) {
...
async function installPythonDependencies(): Promise<boolean> {
  ...
  if (!pythonSrcDir) {
    console.error("Could not find python source directory in any of:", possiblePaths);
    return false;
  }
  ...
  const result = await runCommand(
    uvPath,
    ["pip", "install", "-r", requirementsPath, "--python", join(venvPath, "bin", "python")],
    { cwd: pythonAppSrcDir }
  );

  if (!result.success) {
    console.error("Failed to install dependencies:", result.error);
    const pipResult = await runCommand(pipPath, ["install", "-r", requirementsPath], {
      cwd: pythonAppSrcDir,
    });
